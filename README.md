...
| ------------- | ------------- | ------------- |
| `c`  | a+b  | Переменная c будет содержать значение "a+b" в виде строки |
| `d`  | 1+2  |  "+" трактуется как обычный текст, а не как оператор сложения |
| `e`  | 3  | Для выполнения операции сложения используется двойной знак "$" |
...
Добавляем "break" для выхода из цикла, если переменная вернула 0;
```
